...
export const paymentApi = {
  ...
  createGlobal: async (
    ...
    // El servicio responde `{ group: { totalAmount } }`. Si cambiara la forma,
    // se cae al monto solicitado en lugar de mostrar 0.
    const groupTotal = (data as { group?: { totalAmount?: unknown } } | undefined)?.group
      ?.totalAmount;
    return { totalAmount: groupTotal === undefined ? draft.amount : toAmount(groupTotal) };
  },
