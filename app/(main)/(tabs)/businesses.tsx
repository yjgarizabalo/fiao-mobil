...
export default function BusinessesTab() {
  const { businesses, pagination, fetchBusinesses, loadMoreBusinesses, isLoadingMore } = useBusiness();
  ...
  return (
    <Box flex={1} bg="$backgroundLight50">
          ...
          <Box py="$12" alignItems="center">
            <VStack space="sm" alignItems="center">
              ...
              <Text size="sm" color="$textLight400" textAlign="center">
                {searchText ? `No se encontró "${searchText}"` : "Agrega tu primer negocio con el botón de abajo"}
              </Text>
            </VStack>
          </Box>
        }
        ...
        renderItem={({ item: business }) => {
          ...
          return (
            <Pressable onPress={() => router.push({
              ...
              <Box bg="$white" borderRadius={14} borderWidth={1} borderColor="$borderLight100" overflow="hidden"
      ...
      <Box position="absolute" bottom={0} left={0} right={0}
        ...
        <Button size="lg" w="100%" h={52} borderRadius={14} bg={Colors.primary}
